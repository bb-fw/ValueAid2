...
let _pickerCurrentSet = null;
let _pickerAllItems = null;

function openPicker(title, items, selectedSet, onToggle, onCustomAdd, onDone) {
  _pickerToggle = onToggle;
  _pickerAddCustom = onCustomAdd;
  _pickerCurrentSet = selectedSet;
  ...
  const customRow = document.getElementById('picker-custom-row');
  if (customRow) customRow.style.display = onCustomAdd ? 'flex' : 'none';
  const ci = document.getElementById('picker-custom-inp');
  if (ci) ci.value = '';

  _renderPickerItems(items, selectedSet, '');

  document.getElementById('picker-search').oninput = e => _renderPickerItems(items, selectedSet, e.target.value);

  const doneBtn = document.getElementById('picker-done-btn');
  if (doneBtn) doneBtn.onclick = () => { closeOv('picker-sheet'); if (onDone) onDone(); };

  openOv('picker-sheet');
}

function _renderPickerItems(items, selectedSet, query) {
  ...
  filtered.forEach(item => {
    const sel = selectedSet.has(item);
    ...
    div.addEventListener('click', () => {
      ...
      if (_pickerToggle) _pickerToggle(item, !wasSelected);
    });
    listEl.appendChild(div);
  });
}

function pickerAddCustomItem() {
  const inp = document.getElementById('picker-custom-inp');
  const val = (inp?.value || '').trim();
  if (!val) return;
  if (_pickerAddCustom) _pickerAddCustom(val);
  inp.value = '';
  // Add to current items and re-render
  if (_pickerAllItems && !_pickerAllItems.includes(val)) _pickerAllItems.push(val);
  if (_pickerCurrentSet) _pickerCurrentSet.add(val);
  _renderPickerItems(_pickerAllItems || [], _pickerCurrentSet || new Set(), document.getElementById('picker-search')?.value || '');
}
